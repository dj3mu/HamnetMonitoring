...
namespace SnmpAbstraction
{
    ...
    internal static class SnmpLowerLayerExtensions
    {
        ...
        public static int QueryAsInt(this ISnmpLowerLayer lowerLayer, Oid oid, string what)
        {
            ...
            {
                log.Debug($"SNMP Exception querying '{what}' from '{lowerLayer.Address}' as integer", snmpException);
            }
            catch(HamnetSnmpException hamnetSnmpException)
            {
                log.Debug("Hamnet SNMP Exception querying '{what}' from '{lowerLayer.Address}' as integer", hamnetSnmpException);
            }
